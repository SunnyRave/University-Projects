...
def is_good_prime(number):
    ...
    #not false or if true
        ...
        if(0 in Digits):
            ...
            #if not a single digits, whose return statement is already taken care of. 
            #if prime will return in the last statement
            
            if(len(Digits)!=1):
                #checks for repeat digits by checking the next digits for each number
                #if it fails it returns false
                #if it does not it returns true in the last statement
